...
@Component({
    ...
    styleUrls: [`./my-date-picker.dark.component.css`],
    templateUrl: `./my-date-picker.dark.component.html`,
...
export class MyDatePicker implements OnChanges, ControlValueAccessor, OnDestroy {
    ...
    @Input()
    // 相对时间的时候 跨天之后 时间空间日期要改变
    set updateSelectDate(value: IMyDate) {
        if (value) {
            this.selectedDate = value;
        }
    }
    @Input() generateYearsSum: number = 12;
    @Input() generateYearsRow: number = 3;

    @Output() dateChanged: EventEmitter<any> = new EventEmitter<any>();
    @Output() inputFieldChanged: EventEmitter<any> = new EventEmitter<any>();
    @Output() calendarViewChanged: EventEmitter<any> = new EventEmitter<any>();
    @Output() calendarToggle: EventEmitter<number> = new EventEmitter<number>();
    @Output() inputFocusBlur: EventEmitter<any> = new EventEmitter<any>();
    ...
    selectedDate: IMyDate = {year: 0, month: 0, day: 0, hour: 0, minute: 0, second: 0};
    weekDays: Array<string> = [];
    dates: Array<IMyWeek> = [];
    months: Array<Array<IMyCalendarMonth>> = [];
    years: Array<Array<IMyCalendarYear>> = [];
    selectionDayTxt: string = "";
    selectionDayRelativeTxt: string = "";
    ...
    opts: IMyOptions = {
        ...
        disableUntil: <IMyDate> {year: 0, month: 0, day: 0, hour: 0, minute: 0, second: 0},
        disableSince: <IMyDate> {year: 0, month: 0, day: 0, hour: 0, minute: 0, second: 0},
    ...
    onPrevYears(event: any, year: number): void {
        event.stopPropagation();
        event.preventDefault();
        this.generateYears(Number(year) - this.generateYearsSum);
    }
     ...
     * 如果 [0][0] = 0
     * 那么 [5][0] = 20 = sum - row
     * [0...5][0] = {0, 4, 8, 12, 16, 20}
     * @param year number
     */
    generateYears(year: number): void {
        this.years.length = 0;
        let today: IMyDate = this.getToday();
        let _row = this.generateYearsRow;
        let _sum = this.generateYearsSum;
        let _span = _sum - _row;
        let _col = _sum / _row;
        for (let i = year; i <= _span + year; i += _row) {
            let row: Array<IMyCalendarYear> = [];
            for (let j = i; j < i + _row; j++) {
                let disabled: boolean = this.utilService.isMonthDisabledByDisableUntil({year: j, month: this.visibleMonth.monthNbr, day: this.daysInMonth(this.visibleMonth.monthNbr, j)}, this.opts.disableUntil)
                 || this.utilService.isMonthDisabledByDisableSince({year: j, month: this.visibleMonth.monthNbr, day: 1}, this.opts.disableSince);
                let minMax: boolean = j < this.opts.minYear || j > this.opts.maxYear;
                row.push({year: j, currYear: j === today.year, selected: j === this.visibleMonth.year, disabled: disabled || minMax});
        ...
        this.prevYearsDisabled = this.years[0][0].year <= this.opts.minYear || this.utilService.isMonthDisabledByDisableUntil({year: this.years[0][0].year - 1, month: this.visibleMonth.monthNbr, day: this.daysInMonth(this.visibleMonth.monthNbr, this.years[0][0].year - 1)}, this.opts.disableUntil);
        this.nextYearsDisabled = this.years[_col - 1][_row - 1].year >= this.opts.maxYear || this.utilService.isMonthDisabledByDisableSince({year: this.years[_col - 1][_row - 1].year + 1, month: this.visibleMonth.monthNbr, day: 1}, this.opts.disableSince);
    }

    onUserDateInput(value: string): void {
        // this.inputMode = 'input';
        console.log('onUserDateInput', value, value.length);
        if (value.length === 0) {
            ...
            let absDate: IMyDate = this.utilService.isAbsoluteDateValid(value, this.opts.dateFormat, this.opts.minYear, this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends, this.opts.disableWeekdays, this.opts.disableDays, this.opts.disableDateRanges, this.opts.monthLabels, this.opts.enableDays);
            let relDate: IMyDate = this.utilService.isRelativeDateValid(value, this.startOrEnd, this.opts.dateFormat, this.opts.minYear, this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends, this.opts.disableWeekdays, this.opts.disableDays, this.opts.disableDateRanges, this.opts.monthLabels, this.opts.enableDays);
            let targetDate = null;
            if (this.utilService.isInitializedDate(absDate)){
                targetDate = absDate;
                this.selectionDayRelativeTxt = '';
                this.isRelativeDate = false;
            } else if (this.utilService.isInitializedDate(relDate)){
                targetDate = Object.assign(relDate, {relativeTime: value});
                this.selectionDayRelativeTxt = value;
                this.isRelativeDate = true;
            } else {
                console.log('onUserDateInput invalid');
                this.invalidInputFieldChanged(value);
                return;
            }
            // #debugger
            if (!this.utilService.isSameDate(targetDate, this.selectedDate)) {
                // console.log('onUserDateInput selectDate ==>', targetDate);
                this.selectDate(targetDate, CalToggle.CloseByDateSel);
            }
            else {
                this.updateDateValue(targetDate);
            }
        }
    }
    ...
    onBlurInput(event: any): void {
        ...
        if (this.isRelativeDate){
            this.selectionDayRelativeTxt = event.target.value;
        } else {
            this.selectionDayRelativeTxt = '';
        }
        this.onTouchedCb();
    ...
    invalidInputFieldChanged(value: string): void {
        console.log('invalidInputFieldChanged ==>', value);
        this.invalidDate = value.length > 0;
        this.inputFieldChanged.emit(null);
    ...
    writeValue(value: any): void {
        console.log('[%s] writeValue', this.startOrEnd , value);
        // 绝对时间
        let aRelativeTime = null;
        if (value && (value["date"] || value["jsdate"] || value["formatted"])) {
            ...
            this.selectionDayTxt = this.utilService.formatDate(this.selectedDate, this.opts.dateFormat, this.opts.monthLabels);
            this.selectionDayTxt = this.selectionDayTxt;
            this.selectionDayRelativeTxt = "";
            this.isRelativeDate = false;
        }else if (value === null || value === "") {
            this.selectedDate = {year: 0, month: 0, day: 0, hour: 0, minute: 0, second: 0};
            this.selectionDayTxt = "Invalid Date";
            this.selectionDayRelativeTxt = "Invalid Date";
            this.isRelativeDate = false;
        }else if (aRelativeTime = this.utilService.syntax(value) ){
            let date = this.utilService.formatPhrase(aRelativeTime, this.startOrEnd, this.opts.dateFormat);
            this.selectionDayTxt = date.txt;
            this.selectionDayRelativeTxt = value;
            this.selectedDate = date.dateObj;
            this.isRelativeDate = true;
        }else if (typeof value === 'string'){
            this.selectedDate = this.utilService.parseTimeStringtoObj(value);
            this.selectionDayRelativeTxt = "";
            this.selectionDayTxt = value;
            this.isRelativeDate = false;
        }
        let modelDate = this.getDateModel(this.selectedDate);
        this.inputFieldChanged.emit(modelDate);
    ...
    onCellClicked(cell: any): void {
        console.log('onCellClicked ==>', cell);
        this.isRelativeDate = false;
        cell.dateObj = (Object).assign(this.selectedDate , cell.dateObj);
    ...
    clearDate(): void {
        // Clears the date
        this.updateDateValue({year: 0, month: 0, day: 0, hour: 0, minute: 0, second: 0});
    ...
    decreaseIncreaseDate(decrease: boolean): void {
        ...
        if (this.utilService.isInitializedDate(date)) {
            let d: Date = this.getDate(date.year, date.month, date.day, date.hour, date.minute, date.second);
    ...
    updateDateValue(date: IMyDate, mode ?: any): void {
        ...
        if (!this.opts.inline) {
            let modelDate = this.getDateModel(this.selectedDate);
            this.selectionDayTxt = clear ? "" : this.utilService.formatDate(date, this.opts.dateFormat, this.opts.monthLabels, mode);
            if (this.utilService.isInitializedDate(this.selectedDate)) {
                this.inputFieldChanged.emit(modelDate);
                this.invalidDate = false;
            } else {
    ...
    getDateModel(date: IMyDate): IMyDateModel {
        ...
        return {date: date, isRelative: this.isRelativeDate, jsdate: this.getDate(date.year, date.month, date.day, date.hour, date.minute, date.second), formatted: this.utilService.formatDate(date, this.opts.dateFormat, this.opts.monthLabels), epoc: Math.round(this.getTimeInMilliseconds(date) / 1000.0)};
    }
    ...
    getTimeInMilliseconds(date: IMyDate): number {
        return this.getDate(date.year, date.month, date.day, date.hour, date.minute, date.second).getTime();
    }
    ...
    getDate(year: number, month: number, day: number, hour?: number, minute?: number, second?: number): Date {
    ...
    generateCalendar(m: number, y: number, notifyChange: boolean): void {
        ...
        if (notifyChange) {
            // Notify parent
            let modelDate = this.getDateModel(this.selectedDate);
            this.calendarViewChanged.emit(modelDate);
        }
    }
